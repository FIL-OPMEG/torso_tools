...
switch cfg.method
    ...
    case 'bem_hbf'
        if isfield(data, 'pos') && isfield(data, 'tri')
            ...
            error('Please manually prepare meshes for bem_hbf method!')
        end
        ntissues = numel(geometry);
        if sum(size(cfg.conductivity) == [2 ntissues]) ~= 2
            error('cfg.conductivity needs to be an array of size 2-by-%d',ntissues)
        end
        headmodel = ft_headmodel_bemhbf(geometry, 'conductivity', cfg.conductivity, 'isolatedsource', cfg.isolatedsource);
